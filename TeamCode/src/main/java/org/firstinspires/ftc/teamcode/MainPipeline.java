...
public class MainPipeline extends OpenCvPipeline {
    ...
    public Mat processFrame(Mat input) {
        ...
        int y1[] = {0, 0, 0};
        int x2[] = {0, 0, 0};
        int y2[] = {0, 0, 0};//can use 2d arrays
        ...
        Mat region1_Cr, region2_Cr, region3_Cr;//3 regions to test for RED shipping element
        ...
        YCbCr looks at blueness and redness (and brightness, but you can ignore brightness because that is a seperate variable)
        https://tvone.com/tech-support/faqs/120-ycrcb-values-for-various-colors godsend
         */
        avg_redLeft = (int) Core.mean(region1_Cr).val[0];
        avg_redMiddle = (int) Core.mean(region2_Cr).val[0];
        avg_redRight = (int) Core.mean(region3_Cr).val[0];
        ...
        int max = Math.max(Math.max(avg_redLeft, avg_redMiddle), avg_redRight);
